...
GitLab, and Gitea raw-file routes. For a local directory in automatic mode, OpenCode tries
`.opencode/marketplace.json`, `marketplace.json`, `.agents/plugins/marketplace.json`, and the legacy-compatible
`.claude-plugin/marketplace.json`. Codex catalogs and their `.codex-plugin/plugin.json` manifests are adapted to the
same OpenCode install plan; bundled skills and `.mcp.json` servers are materialized normally. Use `--name` to set a
