...
This starts a local server on `127.0.0.1`, using port `4096` when available and otherwise falling back to a free port, then opens OpenCode in your default browser.
...
By default, OpenCode tries port `4096` and falls back to a free port. You can specify a port:
