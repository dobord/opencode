...
OpenCode exposes built-in tools based on the active client, model, provider, and feature flags. Once a tool is exposed, [permissions](/docs/permissions) determine whether it runs automatically, asks for approval, or is denied. Most permissions default to `allow`, with documented safety exceptions.
...
The following user-facing built-ins are available in OpenCode. Some are conditional: for example, web search depends on the provider or web-search flags, LSP and code mode are experimental, and the question tool depends on the client.

:::note
For file changes, OpenCode exposes either `edit`/`write` or `apply_patch` based on the selected model. All three are controlled by the `edit` permission.
:::
...
Launch a configured subagent to perform a focused unit of work. The tool accepts a subagent type, a prompt, and an optional existing task ID to resume. Background execution is available when the background-subagent experiment is enabled.
...
The Task tool only advertises subagents that are not denied by the current agent's `task` permission. Pattern rules can allow or deny individual subagent names.
