...
Build is the **default** primary agent. It uses the normal configured permission defaults and enables the interactive question and plan-entry actions. This is the standard agent for development work.
...
A restricted agent designed for planning and analysis. Plan denies the `edit` permission for normal project files, including writes and patches. It may write Markdown plans under `.opencode/plans/` and OpenCode's managed plan directory.

Other permissions inherit the normal defaults and your overrides. In particular, `bash` is allowed by default unless you restrict it in your configuration.

Use this agent when you want the LLM to analyze code, suggest changes, or maintain an explicit plan without editing the rest of your codebase.
...
A fast agent specialized for exploring codebases. Its direct file-edit tools are disabled, and its default toolset is focused on searching, reading, and research. Use this when you need to quickly find files by patterns, search code for keywords, or answer questions about the codebase.
...
This option is optional, but it is strongly recommended for subagents. The Task tool uses descriptions to decide when an agent is appropriate; an agent without a description remains available for manual invocation.
...
Use `variant` to select a provider-specific default model variant for the agent, such as a reasoning-effort preset. This setting applies when the agent also has its own `model` configured.
...
Available variant names depend on the provider and model.
...
- **Build agent**: Standard development work using your configured permissions
- **Plan agent**: Analysis and planning with normal project edits denied
