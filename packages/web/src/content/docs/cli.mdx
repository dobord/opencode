...
| <nobr><code>{"--no-replay"}</code></nobr>    |       | Disable mini history replay on resume and after resize (requires `--mini`) |
| <nobr><code>{"--replay-limit"}</code></nobr> |       | Keep only the newest N messages in mini replay (requires `--mini`)         |
...
The canonical command for managing AI providers and credentials is `providers`. `auth` remains an alias for compatibility.
...
OpenCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode providers login` (or `opencode auth login`) to configure API keys for any provider you'd like to use. Credentials are stored in `auth.json` inside the OpenCode data directory; on Linux this is typically `~/.local/share/opencode/auth.json`.

```bash
opencode providers login [url]
```

The optional `url` is for a custom auth provider that publishes login metadata at `/.well-known/opencode`. When OpenCode starts, it also discovers provider credentials from supported environment variables and project `.env` files.
...
Without arguments, this command guides you through adding either a local or remote MCP server.

To configure a server non-interactively, provide a name and either a remote URL or a local command after `--`:

```bash
# Remote server
opencode mcp add docs --url https://example.com/mcp --header "Authorization=Bearer $TOKEN"

# Local server
opencode mcp add filesystem --env "ROOT=/workspace" -- npx -y @modelcontextprotocol/server-filesystem /workspace
```

Non-interactive additions are written to the global config. Use either `--url` or a command after `--`, not both.
...
| <nobr><code>{"--env"}</code></nobr>    | Repeatable `KEY=VALUE` environment entry for a local MCP server |
| <nobr><code>{"--header"}</code></nobr> | Repeatable `KEY=VALUE` HTTP header for a remote MCP server      |
...
| <nobr><code>{"--port"}</code></nobr>        |       | Local server port (tries 4096, then falls back to a free port)             |
...
| <nobr><code>{"--interactive"}</code></nobr> | `-i`  | Run in direct interactive split-footer mode                                |
| <nobr><code>{"--auto"}</code></nobr>        |       | Auto-approve permissions that are not explicitly denied                    |
...
| `OPENCODE_DISABLE_PROJECT_CONFIG`     | boolean | Skip project config, instructions, and `.opencode` discovery               |
| `OPENCODE_DISABLE_AUTOUPDATE`         | boolean | Disable automatic update checks                                            |
| `OPENCODE_ALWAYS_NOTIFY_UPDATE`       | boolean | Emit an update-available event instead of auto-updating                    |
| `OPENCODE_DISABLE_PRUNE`              | boolean | Disable pruning of old data                                                |
| `OPENCODE_DISABLE_TERMINAL_TITLE`     | boolean | Disable automatic terminal title updates                                   |
| `OPENCODE_PERMISSION`                 | string  | Inlined json permissions config                                            |
| `OPENCODE_DISABLE_DEFAULT_PLUGINS`    | boolean | Disable default plugins                                                    |
| `OPENCODE_DISABLE_EXTERNAL_SKILLS`    | boolean | Disable compatibility discovery from `.claude/skills` and `.agents/skills` |
...
| `OPENCODE_ENABLE_EXA`                 | boolean | Prefer Exa as the built-in web-search provider                             |
| `OPENCODE_ENABLE_PARALLEL`            | boolean | Prefer Parallel as the built-in web-search provider                        |
| `OPENCODE_WEBSEARCH_PROVIDER`         | string  | Force the built-in web-search provider to `exa` or `parallel`              |
| `OPENCODE_ENABLE_QUESTION_TOOL`       | boolean | Expose the question tool to non-interactive clients                        |
| `OPENCODE_SERVER_PASSWORD`            | string  | Enable basic auth for `serve`/`web`                                        |
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `opencode`)                          |
| `OPENCODE_MODELS_URL`                 | string  | Custom URL for fetching models configuration                               |
| `OPENCODE_MODELS_PATH`                | string  | Load models configuration from a local file                                |

#### Advanced diagnostics and storage

| Variable                      | Type    | Description                                         |
| ----------------------------- | ------- | --------------------------------------------------- |
| `OPENCODE_DB`                 | string  | Override the database path; supports `:memory:`     |
| `OPENCODE_DISABLE_FFF`        | boolean | Force filesystem search to use the ripgrep fallback |
| `OPENCODE_SHOW_TTFD`          | boolean | Show time-to-first-display diagnostics              |
| `OPENCODE_AUTO_HEAP_SNAPSHOT` | boolean | Write a heap snapshot when RSS exceeds 2 GiB        |
...
| `OPENCODE_EXPERIMENTAL_EXA`                     | boolean | Legacy alias for `OPENCODE_ENABLE_EXA`             |
...
| `OPENCODE_EXPERIMENTAL_CODE_MODE`               | boolean | Enable code mode tools                             |
...
| `OPENCODE_EXPERIMENTAL_WEBSOCKETS`              | boolean | Enable the experimental OpenAI WebSocket transport |
| `OPENCODE_EXPERIMENTAL_PARALLEL`                | boolean | Legacy alias for `OPENCODE_ENABLE_PARALLEL`        |
