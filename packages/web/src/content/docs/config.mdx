...
7. **Active Console organization config** - organization settings for the signed-in account
8. **Managed config files** (system-wide managed directory) - admin-controlled
...
Set the runtime log level with `logLevel`. Supported values are `DEBUG`, `INFO`, `WARN`, and `ERROR`.
...
Set `username` to change the name displayed for you in conversations. This is separate from `OPENCODE_SERVER_USERNAME`, which configures HTTP Basic authentication.
...
OpenCode allows most operations by default, but safety-sensitive actions are not universally auto-approved. `doom_loop` and `external_directory` default to `ask`, and reading `.env` files also asks for approval. You can change these rules with the `permission` option.
...
Use `tool_output` to control when large tool results are truncated in the model-visible response. When either threshold is exceeded, OpenCode writes the full output to its truncation directory and returns a preview.
...
- `max_bytes` - Maximum number of bytes before truncation (default: `51200`).
...
- `tail_turns` - Number of recent user turns, with their assistant and tool responses, to keep verbatim (default: `2`). Set to `0` to disable verbatim tail preservation.
- `preserve_recent_tokens` - Maximum token budget for the preserved recent turns. If omitted, OpenCode derives a budget from the model context window and clamps it between 2000 and 8000 tokens.
...
| Key                     | Description                                                  |
| ----------------------- | ------------------------------------------------------------ |
| `openTelemetry`         | Emit OpenTelemetry spans for AI SDK calls                    |
| `primary_tools`         | Tool IDs that should only be available to primary agents     |
| `continue_loop_on_deny` | Continue the agent loop after a denied tool call             |
| `mcp_timeout`           | Timeout in milliseconds for MCP requests                     |
| `policies`              | Policy statements for supported resources, such as providers |
